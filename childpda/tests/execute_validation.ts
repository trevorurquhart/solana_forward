...
describe("execute validation tests", () => {
    ...
    it("Should not transfer sol to an invalid destination", async () => {
        ...
        try {
            await execute(payer, program, connection, forwardAccount.publicKey, forwardPda, invalidDestination)
    ...
    it("Should error if the token program id is incorrect", async () => {
        ...
        try {
            ...
            await executeWithTokens(payer, program, connection, forwardAccount.publicKey, forwardPda, destination, notTheTokenProgram, mint, forwardAta, destinationAta);
        } catch (e) {
    ...
    it("Should not transfer tokens from an invalid ATA", async () => {
        ...
        try {
            await executeWithTokens(payer, program, connection, forwardAccount.publicKey, forwardPda, destination, TOKEN_PROGRAM_ID, mint, invalidForwardAta.publicKey, destinationAta);
        } catch (e) {
    ...
    it("Should not transfer tokens to an invalid ATA", async () => {
        ...
        let invalidDestination = Keypair.generate();
        try {
            await executeWithTokens(payer, program, connection, forwardAccount.publicKey, forwardPda, destination, TOKEN_PROGRAM_ID, mint, forwardAta, invalidDestination.publicKey);
        } catch (e) {
    ...
    it ("Should not transfer from an invalid forward", async () => {
        ...
        try {
            await execute(payer, program, connection, invalidForward.publicKey, forwardPda, destination);
        } catch (e) {
            expect(e.message).to.contain("incorrect program id for instruction")
            return;
        }
    ...
    it("Should error if the number of accounts is invalid", async () => {
        ...
        let forwardAta = await createAndFundAta(connection, payer, mint, mintAuthority, forwardAmount, forwardPda);
        try {
            await executeWithTokens(payer, program, connection, forwardAccount.publicKey, forwardPda, destination, TOKEN_PROGRAM_ID, mint, forwardAta);
        } catch (e) {
